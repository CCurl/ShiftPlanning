...
namespace ShiftPlannerLib
{
	 ...
	 */
        ...
        public APIResponse getReportsSchedule(RequestFields filter)
        {
            // get report of schedules
            RequestFields requestFields = new RequestFields();
            requestFields.Add("module", "reports.schedule");
            requestFields.Add("method", "GET");
            foreach (var fld in filter)
                requestFields.Add(fld.Key, fld.Value);
            this.setRequest(requestFields);
            return response;
        }
